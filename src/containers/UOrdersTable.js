...
import {getOrders, setStartDate, setOrderId, setFilter} from '../actions/stationAdminActions';
...
var socket;// = new WebSocket('ws://ss.com/api/ws');
...
export default class MOrdersTable extends React.Component{
	...
	componentDidMount(){
        ...
        var startobj = JSON.stringify({action:"GET_ALL_USER_ORDERS", accessToken: localStorage.getItem('access_token')/*, data: JSON.stringify({
            startDate: '',
            endDate: '' 
            })*/
        });
        ...
        $(this.refs.datepicker4).change(()=>{
            if(socket.readyState == 1){
                if(this.refs.datepicker3.value == ''){
                    obj = JSON.stringify({action:"GET_ALL_USER_ORDERS", accessToken: localStorage.getItem('access_token'), data: JSON.stringify({
                                startDate: today,
    ...
    orderClick(id, index){
        ...
        setTimeout(()=>{this.props.dispatch({type: 'ROUTING',
                                        payload:{
                                        method: 'replace',
                                        nextUrl: 'userorders/orderdetails'
                                        }
                                    })}, 600
        )
